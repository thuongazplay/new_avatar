...
public class ShopNpcHandler {
    ...
    public static void displayUI(User us,int npcID, int... itemIds) {
        ...
        List<BossShopItem> NpcShop = PartManager.getInstance().getShopNpcItems()
                .stream()
                .filter(NpcItem -> itemIdList.contains(NpcItem.getItem().getId()))
                .sorted(Comparator.comparingInt(item -> {
                    int index = itemIdList.indexOf(item.getItem().getId());
                    return index == -1 ? Integer.MAX_VALUE : index;
                }))
                .collect(Collectors.toList());
        us.setBossShopItems(NpcShop);
        System.out.println("Danh sách tất cả item trong NpcShop:");
for (BossShopItem item : PartManager.getInstance().getShopNpcItems()) {
    System.out.println("Item ID: " + item.getItem().getId());
}
        service.openUIShopNpc(
                BossShop.builder()
